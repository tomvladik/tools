...
def compute_photo_and_fade(
    slideshow_duration: float,
    num_photos: int,
    requested_photo: float,
    requested_fade: float,
) -> tuple:
    ...
    max_per_photo = (
        slideshow_duration / num_photos if slideshow_duration > 0 else requested_photo
    )
    photo_duration = (
        min(requested_photo, max_per_photo) if max_per_photo > 0 else requested_photo
    )
...
def render_video_with_libopenshot(
    ...
    if no_repeat:
        n = len(photos)
        if n > 0:
            total_fade = fade_duration * (n - 1)
            per_photo = (slideshow_duration + total_fade) / n
            effective_photo_duration = per_photo
            effective_fade_duration = fade_duration
        else:
            effective_photo_duration, effective_fade_duration = (
                photo_duration,
                fade_duration,
            )
    else:
    ...
    while current_time < slideshow_start + slideshow_duration:
        if no_repeat and photo_index >= len(photos):
            break
    ...
    timeline = openshot.Timeline(
        width, height, openshot.Fraction(fps, 1), 44100, 2, openshot.LAYOUT_STEREO
    )
    ...
    for idx, item in enumerate(photo_schedule):
        ...
        if idx > 0:
            ...
            photo_clip.alpha.AddPoint(
                int(effective_fade_duration * 30), 1.0
            )  # Fade to opaque
    ...
    try:
        for frame_num in range(1, total_frames + 1):
            ...
            if frame_num - last_frame_reported >= 100:
                ...
                print(
                    f"  Rendering: {percentage}% (frame {frame_num}/{total_frames})",
                    flush=True,
                )
...
def run_ffmpeg_with_progress(
    cmd: List[str], total_duration: float, description: str = "Processing"
):
        ...
        if frame_match:
            ...
            if current_frame - last_frame_reported >= 100:
                if total_frames > 0:
                    ...
                    print(
                        f"  {description}: {percentage}% (frame {current_frame}/{total_frames})",
                        flush=True,
                    )
                else:
...
def render_video(
    ...
    try:
        ...
        if len(photo_schedule) == 1:
            ...
            cmd.extend(
                [
                    ...
                    "1",
                    "-t",
                    str(photo_schedule[0]["duration"]),
                    "-i",
                    photo_schedule[0]["file"],
                    "-vf",
                    f"scale={width}:{height}:force_original_aspect_ratio=decrease,pad={width}:{height}:(ow-iw)/2:(oh-ih)/2:color={bg_color}",
                    "-r",
                    str(fps),
                    "-c:v",
                    encoder,
            ...
            for i in range(1, len(photo_schedule)):
                ...
                cumulative_offset += (
                    photo_schedule[i - 1]["duration"] - effective_fade_duration
                )
        ...
        actual_duration = (
            float(probe_result.stdout.strip()) if probe_result.returncode == 0 else 0
        )
        ...
        if intro_duration > 0 or outro_duration > 0:
            ...
            full_filter = (
                ";".join(filter_parts + [concat_filter])
                if filter_parts
                else concat_filter
            )
...
def create_openshot_project(
    ...
    if no_repeat:
        n = len(photos)
        if n > 0:
            total_fade = crossfade_duration * (n - 1)
            per_photo = (slideshow_duration + total_fade) / n
            effective_photo_duration = per_photo
            effective_fade_duration = crossfade_duration
        else:
            effective_photo_duration, effective_fade_duration = (
                photo_duration,
                crossfade_duration,
    ...
    project_data = {
        "id": str(uuid.uuid4()),
        "version": {"openshot-qt": "3.0.0", "libopenshot": openshot_version},
        "clips": [],
        "effects": [],
        "files": [],
    ...
    def _build_clip_json_with_libopenshot(
        ...
        try:
            ...
            data["has_audio"] = {
                "Points": [
                    {"co": {"X": 1, "Y": 1 if has_audio else 0}, "interpolation": 0}
                ]
            }
            data["has_video"] = {
                "Points": [
                    {"co": {"X": 1, "Y": 1 if has_video else 0}, "interpolation": 0}
                ]
            }
    ...
    for idx in range(len(clip_objects) - 1):
        ...
        transition_data = {
            ...
            "reader": {"type": "QtImageReader", "path": "@transitions/common/fade.svg"},
            "replace_image": False,
        }
...
Examples:
  ...
  python generate_openshot_project.py --export-video --photo-duration 60 song.wav ./images/ video.mp4
    ...
    parser.add_argument(
        "output_project",
        help="Path for output file (.osp for project, .mp4/.mkv for video)",
    )
    parser.add_argument(
        "--versions",
        action="store_true",
    ...
    parser.add_argument(
        "--no-repeat",
        action="store_true",
        help="Do not repeat images when all have been used; equally prolong each photo to cover audio length.",
    )
